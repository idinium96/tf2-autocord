...
async def githubupdate():
    ...
    if 'Your branch is up to date with' in updateable:
        ...
        await bot.get_user(owner_id).send(f'There is an update to the repo, I\'m going to try to update you')

        await bot.get_user(owner_id).send('Updating to the newest version of the GitHub repo')
        os.system('cd ' + os.getcwd())
        os.system('git pull')
        await bot.get_user(owner_id).send('You should now be on the most recent version')


@tasks.loop(seconds=10)
async def profitgraphing():
    global currenttime
    global graphplots
    global currenttime
    getcurrenttime()
...
async def update(ctx, *, content: str):
    ...
    while response == 0:
        ...
        if choice == 'y' or choice == 'yes':
            response = 1
            await ctx.send('Sent {} {} to the bot {}'.format(mul, mul2, dscontent))
            global botresp
            botresp = False
            while botresp is False:
                async with ctx.typing():
            ...
            def handle_message(user, message_text):
                ...
                if user.steam_id == bot64id:
                    if 'view it here' not in message_text and 'marked as declined' in message_text:
                        ...
                        elif currenttime == '23:59' and "You've made" in sbotresp:
                            global graphplots
                            graphplots = sbotresp
